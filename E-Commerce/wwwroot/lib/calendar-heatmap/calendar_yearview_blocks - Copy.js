(function ($) {
    ...
    $.fn.calendar_yearview_blocks = function (options) {
        ...
        var start = function () {
            ...
            let end_date = new Date(settings.final_date);

            let endDate = new Date(settings.final_date);
            ...
            var start_weekday = settings.start_monday === true?6:0;
            //for (var i = 0; i < 7; i++) {
            ...
            var m = 0;
            for (var i = 0; i < 31; i++) { // For each week, generate a column
                var g_x = i * step;
                var item_html = '';
                ...
                month_position.push({ month_index: i + 1, x: g_x + 2 });
               
                
                
                for (var j = 0; j < 12; j++) { // For each weekday, generate a row
                    ...
                    if (j == 11 & i == 29) {
                        var year = endDate.toLocaleDateString('fa-IR').split('/')[0]
                    ...
                    if (items.length === 2) { // Fill a trangle for the 2nd
                        ...
                        item_html += '<polygon points="' + 0 + ',' + (y + 5.5) + ' ' + 0 + ',' + y + ' ' + 11 + ',' + y + ' ' + 11 + ',' + (y + 5.5)+ '" fill="' + color_1 + '" data-items="' + items_str + '" data-legend="' + legend + '" data-date="' + data_date + '"/>';
            ...
            for (var i = 0; i < month_position.length; i++) {
                ...
                loop_html += '<text x="' + item.x + '" y="-5" class="month">' + (i+1) + '</text>';
            }

            for (var i = 0; i < settings.month_names.length; i++) {

                loop_html += '<text text-anchor="middle" class="wday" dx="-15" dy="'+ (13 * i+7)+'">{0}</text>'.formatString(settings.month_names[i])                
            }
            ...
            //        '<text text-anchor="middle" class="wday" dx="-15" dy="36">{0}</text>'.formatString(settings.day_names[1]) +
            //        '<text text-anchor="middle" class="wday" dx="-18" dy="61">{0}</text>'.formatString(settings.day_names[2]) +
            //        '<text text-anchor="middle" class="wday" dx="-12" dy="86">{0}</text>'.formatString(settings.day_names[3]);
            //} else {
            //    loop_html += '<text text-anchor="middle" class="wday" dx="-15" dy="22">{0}</text>'.formatString(settings.day_names[0]) +
            //        '<text text-anchor="middle" class="wday" dx="-18" dy="48">{0}</text>'.formatString(settings.day_names[1]) +
            //        '<text text-anchor="middle" class="wday" dx="-15" dy="74">{0}</text>'.formatString(settings.day_names[2]);
            //}

            // Fixed size with width= 721 and height = 110
            var wire_html =
                '<svg width="721" height="210">' +
        ...
        var mouseClick = function (evt) {
            ...
            $("#moreinfo").text("On {0}, following items are seen: {1}".formatString(new Date(date).toLocaleDateString('fa-IR'), items))
        };
        ...
        var mouseEnter = function (evt) {
            ...
            var date = $(evt.target).attr('data-date');            
            var text = settings.tooltip_style === 'default' ? "{0}: <br />{1}".formatString(new Date(date).toLocaleDateString('fa-IR'), legend ? legend : items) : (legend ? legend : items);
            ...
            if (items.length >= 1 ||  settings.always_show_tooltip === true) {
                ...
                svg_tip.css({top: target_offset.top*2 - svg_height+5});
                svg_tip.css({left: target_offset.left*2 - svg_width+5});
            }
    ...
    function IsLeapYear(y) {
        ...
        var modulus = y - ((y / 33) * 33);
        var K = false;
        for ( n = 0; n != 8; n++) if (matches[n] == modulus) K = true;
        return K;
    }
